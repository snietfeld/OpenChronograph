...
void MPU9250::initMPU9250()
{
  ...
  delay(100L); // Wait for all registers to reset
...
void MPU9250::calibrateMPU9250(float * gyroBias, float * accelBias)
{
  ...
  delay(40L);  // accumulate 40 samples in 40 milliseconds = 480 bytes
...
Serial.println(ii);
  ...
  delay(25L);  // Delay a while to let the device stabilize
...
void MPU9250::magCalMPU9250(float * bias_dest, float * scale_dest)
{
  ...
  for (ii = 0; ii < sample_count; ii++)
  {
    ...
    if (Mmode == M_8HZ)
    {
      delay(135L); // At 8 Hz ODR, new mag data is available every 125 ms
    }
    if (Mmode == M_100HZ)
    {
      delay(12L);  // At 100 Hz ODR, new mag data is available every 10 ms
    }
  }
